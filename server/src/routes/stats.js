...
exports.statsRoutes.get("/overview", function (req, res) {
  ...
  var inProgressCount = (_c = db.prepare(
    "SELECT COUNT(*) as count FROM appeals a".concat(
      baseWhere,
      " a.status IN ('in_progress', 'mediating', 'escalated')",
    ),
  )).get.apply(_c, params);
