...
exports.statsRoutes.get("/overview", function (req, res) {
  ...
  var extraWhere = districtFilter ? " AND" : " WHERE";
  ...
  var pendingCount = (_b = db.prepare(
    "SELECT COUNT(*) as count FROM appeals a"
      .concat(districtFilter)
      .concat(extraWhere, " status = 'pending'"),
  )).get.apply(_b, params);
  ...
  var completedCount = (_d = db.prepare(
    "SELECT COUNT(*) as count FROM appeals a"
      .concat(districtFilter)
      .concat(extraWhere, " status = 'completed'"),
  )).get.apply(_d, params);
  var overdueCount = (_e = db.prepare(
    "SELECT COUNT(*) as count FROM appeals a"
      .concat(districtFilter)
      .concat(extraWhere, " status = 'overdue'"),
  )).get.apply(_e, params);
  ...
  var mediationCount = (_k = db.prepare(
    "SELECT COUNT(*) as count FROM appeals a"
      .concat(districtFilter)
      .concat(extraWhere, " resolution_channel = 'mediation'"),
  )).get.apply(_k, params);
  var crossDeptCount = (_l = db.prepare(
    "SELECT COUNT(*) as count FROM appeals a"
      .concat(districtFilter)
      .concat(extraWhere, " resolution_channel = 'cross_department'"),
  )).get.apply(_l, params);
  var escalatedCount = (_m = db.prepare(
    "SELECT COUNT(*) as count FROM appeals a"
      .concat(districtFilter)
      .concat(extraWhere, " resolution_channel = 'escalated'"),
  )).get.apply(_m, params);
  var reworkCount = (_o = db.prepare(
    "SELECT COUNT(*) as count FROM appeals a"
      .concat(districtFilter)
      .concat(extraWhere, " need_rework = 1"),
  )).get.apply(_o, params);
